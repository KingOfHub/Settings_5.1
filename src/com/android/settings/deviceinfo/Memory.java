...
import com.android.settings.DockManager;
...
public class Memory extends SettingsPreferenceFragment implements Indexable {
    ...
    public void onCreate(Bundle icicle) {
        ...
        for (StorageVolume volume : storageVolumes) {
        	...
        	Log.d(TAG, volume.getDescription(context));
        	Log.d(TAG,"" + volume.isEmulated());
            if (!volume.isEmulated()) {
                ...
                if(SystemProperties.getBoolean("pwv.docker.support", false) && DockManager.getInstance().isEnable()){
                    if(volume.getPath().equals("/storage/usbotg")){
                        continue;
                    }
                }
                if(SystemProperties.getBoolean("pwv.docker.support", false) && !DockManager.getInstance().isEnable()){
                    if(volume.getPath().equals("/storage/usbotg1")){
                        continue;
                    }
                    if(volume.getPath().equals("/storage/usbotg2")){
                        continue;
                    }
                }
